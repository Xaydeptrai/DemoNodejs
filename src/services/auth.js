...
//npm i bcryptjs dung de ma hoa
//npm i jsonwebtoken de tao token
//npm i http-errors de quan ly loi
//npm i joi de validation
...
export const register = ({email, password}) => new Promise(async (resolve, reject) => {
    try {
        ...
        const refreshToken = response[1] ? jwt.sign({
            id: response[0].id,
        }, process.env.JWT_SECRET_REFRESH_TOKEN, {expiresIn: '15d'}) : null
...
export const login = ({email, password}) => new Promise(async (resolve, reject) => {
    try {
        ...
        const refreshToken = isChecked ? jwt.sign({
            id: response.id,
        }, process.env.JWT_SECRET_REFRESH_TOKEN, {expiresIn: '60s'}) : null

        resolve({
            err: accessToken ? 0 : 1,
            mes: accessToken ? 'Login is successfully' : response ? 'Password is wrong' : 'Email not found',
            'access_token': accessToken ? `Bearer ${accessToken}` : accessToken,
            'refresh_token': refreshToken
        })
        ...
        if (response) {
            jwt.verify(refresh_token, process.env.JWT_SECRET_REFRESH_TOKEN, (err) => {
                if (err) {
                    ...
                    resolve({
                        err: accessToken ? 0 : 1,
                        mes: accessToken ? 'Access token generated' : 'Fail to generate access token',
                        'access_token': accessToken ? `Bearer ${accessToken}` : accessToken,
                        'refresh_token': refreshToken
                    })
                }
